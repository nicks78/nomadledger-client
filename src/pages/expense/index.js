...
class Expense extends Component {
  ...
  handleDuplicateItem = (expense) => {
    this.setState({ receivedAt: null })
    this.props.duplicateItem(this.state.reducer, expense)
  }
                  ...
                  {!isFetching ?
                    this.props.listExpenses.map((expense, index) => {
                      return <TableRow key={index}>
                        <TableCell style={{ width: "10%" }}> <RenderImage img={expense.receipt} /></TableCell>
                        ...
                        <TableCell align="center" style={{ display: "flex", justifyContent: "center" }}>
                          ...
                          <Tooltips title={locale.wording.duplicate}><IconButton aria-label="Duplicate item" onClick={() => { this.handleDuplicateItem(expense) }}><FileCopyIcon style={{ color: "grey", fontSize: 18 }} /></IconButton></Tooltips>
...
export default connect(mapStateToProps, { downloadFile, createItem, getItemList, getItem, createState, resetState, deleteElement, duplicateItem })(Expense);
